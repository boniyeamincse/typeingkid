    ...
    const load = async () => {
      try {
        ...
        const [lessonsRes, summaryRes] = await Promise.all([
          api.get('/lessons?difficulty=advanced'),
          api.get('/lessons/progress/summary?difficulty=advanced'),
        ]);
        ...
        const merged = lessonsRes.data
          ...
          .map((lesson) => {
            ...
            return {
              ...
              path: `/lessons/advanced/${lesson.order_index + 1}`,
            };
          });
        ...
        setError(loadError.response?.data?.message || 'Failed to load advanced lessons.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="bg-rose-500 text-white rounded-3xl p-6 shadow-xl mb-6">
          <p className="uppercase tracking-widest text-xs text-rose-100 font-bold mb-2">Paragraph Training</p>
          <h1 className="text-3xl font-black">Advanced Lessons</h1>
          <p className="text-rose-100 mt-2">Build endurance and control with paragraph-based typing challenges.</p>
        ...
        {!isLoading && !error && items.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-5 text-slate-600 font-semibold">
            No advanced lessons found. Run backend seed to add paragraph lessons.
          </div>
        ) : null}
...
export default AdvancedLessonsPage;
